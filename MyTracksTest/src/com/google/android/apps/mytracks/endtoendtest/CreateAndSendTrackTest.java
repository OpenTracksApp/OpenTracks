...
public class CreateAndSendTrackTest extends ActivityInstrumentationTestCase2<TrackListActivity> {
  ...
  public void testCreateAndSendTrack() {
    ...
    EndToEndTestUtils.findMenuItem(activityMyTracks.getString(R.string.menu_send_google), true, false);
  ...
  public void testCreateTrackWithMarkers() {
    ...
    EndToEndTestUtils.findMenuItem(activityMyTracks.getString(R.string.menu_delete_all), true, false);
    ...
    instrumentation.waitForIdleSync();
    // Check the new track
    assertTrue(EndToEndTestUtils.SOLO.waitForText(EndToEndTestUtils.TRACK_NAME, 1, 5000, true, false));
  }
  ...
  private void createWaypointAndStatistics() {
    sendKeys(KeyEvent.KEYCODE_MENU);
    EndToEndTestUtils.findMenuItem(activityMyTracks.getString(R.string.menu_markers), true, false);
    
    assertTrue(EndToEndTestUtils.SOLO.waitForText(activityMyTracks.getString(R.string.marker_list_empty_message)));

    sendKeys(KeyEvent.KEYCODE_MENU);
    EndToEndTestUtils.findMenuItem(activityMyTracks.getString(R.string.menu_insert_marker), true, true);
    EndToEndTestUtils.SOLO.clickOnText(activityMyTracks.getString(R.string.marker_edit_type_waypoint));
    EndToEndTestUtils.rotateAllActivities();
    EndToEndTestUtils.SOLO.enterText(0, WAYPOINT_NAME);
    EndToEndTestUtils.SOLO.clickOnButton(activityMyTracks.getString(R.string.marker_edit_add));
    assertTrue(EndToEndTestUtils.SOLO.searchText(WAYPOINT_NAME));

    sendKeys(KeyEvent.KEYCODE_MENU);
    EndToEndTestUtils.findMenuItem(activityMyTracks.getString(R.string.menu_insert_marker), true, true);
    EndToEndTestUtils.SOLO.clickOnText(activityMyTracks.getString(R.string.marker_edit_type_statistics));
    EndToEndTestUtils.rotateAllActivities();
