 ...
 */
public class GPXTrackExporterForStrava implements TrackExporter {

    private static final String TAG = GPXTrackExporterForStrava.class.getSimpleName();
    ...
    public GPXTrackExporterForStrava(ContentProviderUtils contentProviderUtils, String creator) {
        ...
        if (printWriter != null) {
            ...
            if (trackPoint.hasSpeed() || trackPoint.hasHeartRate() || trackPoint.hasCyclingCadence() || trackPoint.hasAltitudeGain() || trackPoint.hasAltitudeLoss()) {
                ...
                if (trackPoint.hasSpeed()) {
                    printWriter.println("<speed>" + SPEED_FORMAT.format(trackPoint.getSpeed().toMPS()) + "</speed>");
                }

                if (trackPoint.hasHeartRate()) {
                    printWriter.println("<heartrate>" + HEARTRATE_FORMAT.format(trackPoint.getHeartRate_bpm()) + "</heartrate>");
                }

                if (trackPoint.hasCyclingCadence()) {
                    printWriter.println("<cadence>" + CADENCE_FORMAT.format(trackPoint.getCyclingCadence_rpm()) + "</cadence>");
                }

                if (trackPoint.hasPower()) {
                    printWriter.println("<power>" + POWER_FORMAT.format(trackPoint.getPower()) + "</power>");
                }

                if (trackPoint.hasAltitudeGain()) {
                    printWriter.println("<elevationgain>" + ALTITUDE_FORMAT.format(trackPoint.getAltitudeGain()) + "</elevationgain>");
                }

                if (trackPoint.hasAltitudeLoss()) {
                    printWriter.println("<elevationloss>" + ALTITUDE_FORMAT.format(trackPoint.getAltitudeLoss()) + "</elevationloss>");
                }

                if (trackPoint.hasSensorDistance()) {
                    printWriter.println("<distance>" + DISTANCE_FORMAT.format(trackPoint.getSensorDistance().toM()) + "</distance>");
                }
